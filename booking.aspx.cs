...
public partial class booking : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            try
            {
                ...
                CurrentPageIndex = 0;
                if (!String.IsNullOrEmpty(Request.QueryString["FromDate"].ToString()))
                {
                    checkInTime = Request.QueryString["FromDate"].ToString();
                    this.txtFromDate.Text = checkInTime;
                }
                if (!String.IsNullOrEmpty(Request.QueryString["ToDate"].ToString()))
                {
                    checkOutTime = Request.QueryString["ToDate"].ToString();
                    this.txtToDate.Text = checkOutTime;
                }
                if (!String.IsNullOrEmpty(Request.QueryString["Adults"].ToString()))
                {
                    numAdults = Request.QueryString["Adults"].ToString();
                    this.drpNumberOfAdults.SelectedValue = numAdults;
                }
                if (!String.IsNullOrEmpty(Request.QueryString["kids"].ToString()))
                {
                    numKids = Request.QueryString["kids"].ToString();
    ...
    public int CurrentPageIndex
    {
        get
        {
            ...
            if (Session["pg"] == null)
                ...
                return Convert.ToInt16(Session["pg"]);
        }
        set
        {
            //ViewState["pg"] = value;
            Session.Add("pg", value);
        }
    }
    ...
    {
        Bookings bookings = new Bookings();
        this.NumRooms = SqlInt32.Parse(this.hidNumberOfAdults.Value) + SqlInt32.Parse(this.hidNumberOfChildren.Value);
    ...
    protected void roomTypesList_ItemCommand(object source, DataListCommandEventArgs e)
    {
        ...
        {
            //DataTable dt = GetRoomTypePictures(Convert.ToInt32(e.CommandArgument));
            //rptrImage.DataSource = dt;
            //rptrImage.DataBind();
            //Panel modalPanel = (Panel)e.Item.FindControl("panelModalPopup");

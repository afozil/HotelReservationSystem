...
public partial class Hotel_SalesResevForcast : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Context.User.IsInRole("administrator") || Context.User.IsInRole("sales") || Context.User.IsInRole("frontoffice"))
        {
    ...
    {
        DataTable table;
        if (Context.User.IsInRole("SALES") == true) 
        { 
        table = new Employees().SelectByRoleName("SALES");
        }
        else if (Context.User.IsInRole("frontoffice") == true)
        {
            table = new Employees().SelectByRoleName("frontoffice");
        }
        else if (Context.User.IsInRole("administrator") == true)
        {
            table = new Employees().SelectByRoleName("SALES");
        }
        else
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
       ...
       // EmpID = "1";
        DataTable table = reserv.SelectSalesReservationForcastReport(EmpID, fromDate.ToString(),username);
        if (table.Rows.Count == 0)
        {
        ...
        string popupScript = "<script language='javascript'>" +
                          "window.open('ReservationForcastView.aspx','ThisPopUp1', " +
                          "'left = 300, top=150,  " +
